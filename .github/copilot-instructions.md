## Minerva Ops Copilot Instructions
...
- Build a multi-tenant Payload CMS + Next.js app for Minerva agency; tenants = customer websites.
- Payload uses the multi-tenant plugin; each tenant is isolated, with a superadmin able to manage all.
- Enforce RBAC: customers can only access their own data; superadmin has full control.

## Required references

- Always query the Context7 MCP server for up-to-date docs relevant to Payload, the multi-tenant plugin, Next.js, and related tooling before making tech decisions or writing non-trivial code.
- Check and honor any rules in ./cursor/rules for Payload-specific guidance.

## Implementation guidance

- Keep tenant isolation in mind for collections, access control, and UI flows; prefer per-tenant scoping and guardrails.
- When adding admin features, ensure superadmin override remains intact without weakening tenant boundaries.
- Prefer Payload-native patterns (hooks, access control functions, admin UI config) over ad-hoc code when possible.
- Comment code to explain intent when it aids maintainers; keep comments succinct and purpose-focused.
- Factor shared helpers (auth/RBAC, access control, data shaping, other utilities) into reusable modules under lib instead of re-implementing per collection or feature.

## Testing and quality

- Favor automated coverage for tenant-specific access rules and content flows.
- Validate that multi-tenant behaviors and RBAC constraints are enforced in tests.
